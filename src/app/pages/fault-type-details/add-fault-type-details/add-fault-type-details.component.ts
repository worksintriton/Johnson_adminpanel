...
@Component({
  selector: 'app-add-fault-type-details',
  templateUrl: './add-fault-type-details.component.html',
  styleUrls: ['./add-fault-type-details.component.css']
})
export class AddFaultTypeDetailsComponent implements OnInit {
  isShowErrors: boolean = false;
  @ViewChild(AddEditFaultTypeDetailsFormComponent,{ static: false })
  public addForm: AddEditFaultTypeDetailsFormComponent;
  user: User;
  constructor(public dialogRef: MatDialogRef<AddFaultTypeDetailsComponent>, private authService: AuthenticationService,
  ...
  public save() {
    ...
    if (this.addForm.addEditForm.valid) {
      const enteredData = this.addForm.addEditForm.value;
        this.adminService.createfault(enteredData).subscribe(
          response => {
